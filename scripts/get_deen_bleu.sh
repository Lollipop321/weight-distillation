...
decodes_file=$1
reference_file=$reference_en
sed -i s'/@@ //g' $decodes_file
#detokenize the decodes file to format the manner to do tokenize
perl $detokenizer -l en < $decodes_file > $decodes_file.dtk
#replace unicode
perl $replace_unicode_punctuation -l en < $decodes_file.dtk > $decodes_file.dtk.punc
#tokenize the decodes file by moses tokenizer.perl
perl $tokenizer -l en < $decodes_file.dtk.punc > $decodes_file.dtk.punc.tok
