...
class WDV29MultiheadAttention(nn.Module):
    """Multi-headed attention.

    See "Attention Is All You Need" for more details.
    """

    def __init__(self, embed_dim, num_heads, dropout=0., wd_require_gradient=False, bias=True, add_bias_kv=False, add_zero_attn=False):
        super().__init__()
        ...
        self.q_weight = Parameter(torch.Tensor(embed_dim, embed_dim), requires_grad=wd_require_gradient)
        self.k_weight = Parameter(torch.Tensor(embed_dim, embed_dim), requires_grad=wd_require_gradient)
        self.v_weight = Parameter(torch.Tensor(embed_dim, embed_dim), requires_grad=wd_require_gradient)
        self.tanh_weight_q_weight = Parameter(torch.Tensor(embed_dim, embed_dim))
        self.tanh_weight_k_weight = Parameter(torch.Tensor(embed_dim, embed_dim))
        self.tanh_weight_v_weight = Parameter(torch.Tensor(embed_dim, embed_dim))
        self.tanh_bias_q_weight = Parameter(torch.Tensor(embed_dim, embed_dim))
        self.tanh_bias_k_weight = Parameter(torch.Tensor(embed_dim, embed_dim))
        self.tanh_bias_v_weight = Parameter(torch.Tensor(embed_dim, embed_dim))
        if bias:
            self.q_bias = Parameter(torch.Tensor(embed_dim), requires_grad=wd_require_gradient)
            self.k_bias = Parameter(torch.Tensor(embed_dim), requires_grad=wd_require_gradient)
            self.v_bias = Parameter(torch.Tensor(embed_dim), requires_grad=wd_require_gradient)
        ...
        self.out_proj = fairseq.modules.WDV29Linear(embed_dim, embed_dim, bias=bias)
    ...
    def in_proj_wd(self, input, tanh_weight_weight, tanh_bias_weight, tanh_weight_bias, tanh_bias_bias, weight, bias):
        weight = tanh_weight_weight * torch.tanh(weight) + tanh_bias_weight
        if bias is not None:
            bias = tanh_weight_bias * torch.tanh(bias) + tanh_bias_bias
